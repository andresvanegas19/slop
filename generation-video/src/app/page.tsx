        ...
        <section className={cn("grid min-h-screen min-w-0 place-items-center max-[760px]:min-h-[62vh]", isEditorOpen ? "p-6" : "p-8 max-[760px]:p-[18px]")}>
          ...
          >
            ...
            {hasThread && <ChatThread threadRef={s.threadRef} thread={s.thread} pendingOp={s.pendingOp} isEditing={s.isEditing} onRetry={s.retryThreadOp} onCancel={s.cancelEdit}
              ...
              />}
            <Composer studio={s} />
            {s.isGenerating && !s.isContinueMode && <div className="hidden max-[760px]:mt-[18px] max-[760px]:block"><BlobLoader label={s.generatingKind === "market" ? "Rendering your market update" : s.mediaType === "ad" ? "Creating your ad" : s.mediaType === "company" ? "Creating your company short" : "Preparing your video"} size={140} /><button type="button" className={CANCEL_BUTTON} onClick={s.cancelGeneration}>Cancel</button></div>}
