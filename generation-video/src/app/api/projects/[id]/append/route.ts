...
import { frameFilePath, frameImageUrl, loadProject, ProjectNotFoundError, saveProject, videoUrl, withProjectLock, type ProjectFrame } from "@/lib/projects";
...
/**
 * POST `{ uploadId?, prompt? }` (at least one) → `{ project, appendedFrameIndex, enhancedPrompt?, ragSources? }`.
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!uploadId && !prompt) return NextResponse.json({ error: "Send \"uploadId\" (an uploaded video) or \"prompt\" (a new shot to generate)." }, { status: 400 });
    ...
    logInfo("project_append_started", { projectId: id, mode: upload ? "upload" : "generate" });
    ...
    const message = error instanceof ProjectNotFoundError || error instanceof UploadError
      ? error.message
