...
export function ruleIntent(context: IntentContext): { action: IntentAction; params: IntentParams; strong: boolean } {
  ...
  if (seconds !== undefined) return { action: "append_shot", params: { prompt: text, seconds }, strong: true };
  ...
  if (EXTEND.test(text)) {
    return { action: "append_shot", params: { prompt: text }, strong: true };
  }
...
export async function detectIntent(context: IntentContext): Promise<Intent> {
  ...
  try {
    ...
    if (call.name === "append_shot") {
      params.prompt = text(call.args.prompt) || context.message;
